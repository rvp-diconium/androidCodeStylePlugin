...
class CodeStylePluginTest {
	...
	@Test fun download_dir_valid_folder_passes() {
	...
	@Test(expected = Exception::class) fun download_dir_invalid_folder_throws() {
		val dir = File(testHomeDir, "custom_location_2")
		val config = CodeStyleConfig().apply { downloadDir = dir.path }
		CodeStylePlugin.getDownloadDir(project, config)
	}

	@Test fun getCacheFolder_returns_gradle_cache_folder_if_all_exist() {
	...
	@Test fun getCacheFolder_returns_null_if_useCache_is_false() {
		// prepare file system for test
		val testHome = Files.createTempDirectory(TEST_FOLDER)
			.toFile()
		val gradleFolder = File(testHome, CACHE_FOLDER)
	...
	@Test fun getCacheFolder_returns_null_if_any_does_not_exist() {
		// prepare file system for test
		val testHome = Files.createTempDirectory(TEST_FOLDER)
			.toFile()
		testHome.deleteOnExit()
