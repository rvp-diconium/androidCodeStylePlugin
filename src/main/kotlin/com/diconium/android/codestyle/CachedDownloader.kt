...
class CachedDownloader(
  ...
  private fun needsUpdateCache(cached: File): Boolean {
    return forceOverwrite || !cached.exists() || isOldFile(cached)
  }

  private fun needsUpdateTarget(
    from: File,
    target: File
  ): Boolean {
    return forceOverwrite || !target.exists() || !isTheSame(from, target)
  }
  ...
  companion object {
    ...
    internal fun downloadNow(
      ...
      val downloader = CachedDownloader(
          Helpers.stringValidator, Helpers.folderValidator, Helpers.cacheNameGenerator, fileDownloader,
          Helpers.fileCopier, Helpers.fileMover, Helpers.compareFiles, force, maxCacheAge, logger, outputDir, cacheDir
      )
