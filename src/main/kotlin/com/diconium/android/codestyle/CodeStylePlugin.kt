  ...
  override fun apply(target: Project) {
    ...
    target.afterEvaluate {
      ...
      CachedDownloader.downloadNow(
          config.downloads, cache, false, config.debug, config.maxCacheAge, getDownloadDir(target, config)
      )

      // task is added to the gradle tasks
      // and can be executed manually
      target.tasks.create(
          "downloadCodeStyle", CachedDownloadTask::class.java
      ) {
        group = "help"
